...
class B_line_minimal(Base_agent):
    ...
    def get_action(self, observation, *args, **kwargs):
        ...
        # Ensure observation is exactly 52
        if observation.shape[1] < 52:
            observation = np.pad(observation, ((0, 0), (0, 52 - observation.shape[1])), 'constant')
        elif observation.shape[1] > 52:
            observation = observation[:, :52]

        # Ensure slicing does not break shape
        host_info = observation[:, 1:]  # Slicing reduces size by 1
        if host_info.shape[1] % 3 != 0:
            host_info = np.pad(host_info, ((0, 0), (0, 3 - (host_info.shape[1] % 3))), 'constant')

        host_info = host_info.reshape(observation.shape[0], -1, 3)
